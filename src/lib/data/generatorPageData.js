...
// 			return "This SQL file registers the audio database rows and links.";
// 		}

// 		if (normalizedPath.includes("/art/")) {
// 			return "This SQL file registers the art rows that point at imported assets.";
// 		}
...
// 			return "This SQL file creates the building rows and effect tables the pattern depends on.";
// 		}
...
export const recipeLaunchRecipes = [
	{
		...
		deliverables: [
			...
			"Optional effect rows such as “Building_YieldChanges” or other building tables.",
			"Small Lua validation pattern for checking whether the dummy proxy is present on a city.",
		],
		example: {
	...
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Unit_FreePromotions", "Starter place to attach a free promotion to the replacement unit.", "rows"),
			{
				label: "Civilization Starter",
				href: "/template-generators?generator=civilization-starter",
				note: "The template generator already lays out the civ unique base; use this recipe when you want to deepen or customize the replacement rows beyond the starter output.",
			},
		],
	},
	{
		...
		example: {
			...
			files: [
				...
				snippetFile(
					...
					"Gameplay consumer that demonstrates a once-per-turn capital reward path with a persisted cooldown marker.",
				),
			],
		},
		touchpoints: [
	...
	},
	{
		...
		touchpoints: [
			...
			linkToPage("Free Promotion From Building / Policy / Trait", "/pattern-library", "Best follow-up when the policy payload is not just yields and needs combat-class or promotion wiring."),
		],
	},
	{
	...
	},
	{
		...
		touchpoints: [
			...
			linkToPage("City Capture Follow-Up", "/pattern-library", "Useful companion when the remove example is really part of a broader conquest cleanup flow."),
		],
	},
	{
...
GameEvents.PlayerCanTrain.Add(function(iPlayer, eUnit)
	...
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Buildings", "Per-turn scans often end by granting or syncing a building effect.", "rows"),
		],
	},
	{
...
return resolveCapitalOrCoastalCity`,
		...
		example: {
			...
			files: [
				snippetFile(
					...
					"Route state recipe that turns an active route plus a religion condition into one reward handoff instead of scattering the checks across multiple files.",
				),
			],
		},
		touchpoints: [
	...
	},
	{
		...
		status: "Lua Pattern",
		copy: "Use this when religious pressure itself should be turned into yields, XP, training speed, or city effects. Instead of checking only majority religion, read the pressure numbers directly and convert that live amount into the bonus you want.",
		deliverables: [
...
local function refreshGarrisonPromotion(pUnit)
	...
	},
	{
		...
		touchpoints: [
			{ label: "GameEvents.UnitUpgraded", href: "/lua-api-explorer", note: "Preferred restore hook when the upgraded unit needs its custom state reapplied." },
	...
	},
	{
		...
		deliverables: [
			...
			"Belief schema references for common city and holy-city payload tables.",
			"A clean place to hand off into rewards, dummy buildings, or restrictions.",
		],
		example: {
			title: "Religion gate variants",
			summary:
				"Break the religion logic into a few reusable checks so beginners can see the difference between a city majority gate, a holy-city gate, and a belief-ownership gate before attaching the actual reward.",
			files: [
				snippetFile(
					"Lua/Gameplay/CityMajorityReligionGate.lua",
					"lua",
					"local function cityPassesReligionGate(pCity)\n\tif not pCity then\n\t\treturn false, nil\n\tend\n\tlocal eReligion = pCity:GetReligiousMajority()\n\tif eReligion == nil or eReligion == -1 then\n\t\treturn false, nil\n\tend\n\treturn true, eReligion\nend",
					"Starter gate for mechanics that should only run in cities with an active majority religion, while also returning the resolved religion ID for follow-up reward logic.",
				),
				snippetFile(
					"Lua/Gameplay/HolyCityReligionGate.lua",
					"lua",
					"local function cityIsHolyCityForFounder(pPlayer, pCity)\n\tif not pPlayer or not pCity then\n\t\treturn false, nil\n\tend\n\tlocal eReligion = pPlayer:GetReligionCreatedByPlayer()\n\tif not eReligion or eReligion <= 0 then\n\t\treturn false, nil\n\tend\n\tlocal pHolyCity = Game.GetHolyCityForReligion(eReligion, pPlayer:GetID())\n\tif not pHolyCity then\n\t\treturn false, eReligion\n\tend\n\treturn pHolyCity:GetID() == pCity:GetID(), eReligion\nend",
					"Use this when the mechanic should only open in the founder's holy city instead of any city that happens to follow the religion.",
				),
				snippetFile(
					"Lua/Gameplay/BeliefOwnershipGate.lua",
					"lua",
					"local function religionOwnsBelief(eReligion, eBelief)\n\tif not eReligion or eReligion <= 0 or not eBelief then\n\t\treturn false\n\tend\n\tfor _, beliefID in pairs(Game.GetBeliefsInReligion(eReligion) or {}) do\n\t\tif beliefID == eBelief then\n\t\t\treturn true\n\t\tend\n\tend\n\treturn false\nend\n\nlocal function playerOwnsBelief(pPlayer, eBelief)\n\tif not pPlayer then\n\t\treturn false, nil\n\tend\n\tlocal eReligion = pPlayer:GetReligionCreatedByPlayer()\n\tif not eReligion or eReligion <= 0 then\n\t\treturn false, nil\n\tend\n\treturn religionOwnsBelief(eReligion, eBelief), eReligion\nend",
					"Belief variant for cases where the civ must actually own a specific belief before a special reward path, dummy building, or restriction should activate.",
				),
			],
		},
		touchpoints: [
			...
			linkToLua("City:GetReligiousMajority", "city-getreligiousmajority-233", "methods", "Primary city helper for majority-religion gates and dummy-building sync checks."),
			linkToLua(
				"Player:GetReligionCreatedByPlayer",
				"player-getreligioncreatedbyplayer-490",
				"methods",
				"Resolve the player's founded religion before checking holy-city or belief ownership conditions.",
			),
			linkToLua(
				"Game.GetHolyCityForReligion",
				"game-getholycityforreligion-94",
				"methods",
				"Use this when the gate should only pass for the founder's holy city rather than every converted city.",
			),
			linkToLua("Game.GetBeliefsInReligion", "game-getbeliefsinreligion-58", "methods", "Useful when the gate should open only if the founded religion owns a specific belief."),
	...
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Civilization_CityNames", "City examples often start from a city collection even when the real goal is spatial filtering.", "rows"),
	...
	},
	{
		...
		deliverables: [
			"Plot resource placement scaffold.",
		...
		touchpoints: [
			...
			linkToSchema("Resource_QuantityTypes", "Reference the normal quantity expectations for placed resources.", "rows"),
...
local function spawnUnitWithFallback(pPlayer, pCity)
	...
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Buildings", "Tracked city state often drives whether a city should receive a building effect.", "rows"),
			linkToSchema("Civilization_CityNames", "Useful when city scripts need to compare or report on known city identities.", "rows"),
		],
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Religions", "Religion rows behind the city religious context the unit should inherit.", "rows"),
			linkToSchema("Beliefs", "Belief mechanics often determine why the religious unit is being created in the first place.", "rows"),
			...
			{ label: "Lua API Explorer", href: "/lua-api-explorer", note: "Use the explorer for player, city, and unit helpers that support religion-aware spawn flows." },
			{
	...
	},
	{
		...
		touchpoints: [
			linkToLua("Team:IsHasTech", "team-ishastech-110", "methods", "One of the common team checks that benefits from a safe team resolver."),
	...
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Buildings", "Useful when the trained unit should flip a city dummy building or other marker state.", "rows"),
			{
	...
	},
	{
		...
		touchpoints: [
			...
			{ label: "Unit Tracking Table", href: "/pattern-library", note: "Useful companion when newly created units should be tracked for later cleanup or identity checks." },
			{
				label: "Promotion-On-Train / Promotion-On-Create",
				href: "/pattern-library",
				note: "Read this next when the same post-create promotion rules also need a training hook so both entry paths stay aligned.",
			},
		],
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Buildings", "Common city payload when founding should grant a dummy or free building.", "rows"),
	...
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Buildings", "Most post-capture cleanup uses dummy buildings or city state proxies.", "rows"),
			...
			{ label: "Building Grant / Remove Lua Helper", href: "/pattern-library", note: "Useful companion when capture cleanup mostly consists of city building state changes." },
			{
				label: "City Capture / Found / Construct Hooks",
				href: "/pattern-library",
		...
		example: {
			...
			files: [
				snippetFile(
					...
					"Policy-adoption scaffold for one-time scripted rewards that complement the database policy tables.",
				),
				snippetFile(
	...
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Specialists", "Companion surface when the GP mechanic also mirrors specialist rate or city-state logic.", "rows"),
		],
	},
	{
		...
		status: "High-Use Recipe",
		copy: "Use this when a bonus should exist only while the player is currently inside a Golden Age. The common pattern is to check `GetGoldenAgeTurns() > 0`, sync a hidden city carrier while the state is active, and remove it cleanly as soon as the Golden Age ends.",
		deliverables: [
	...
	},
	{
		...
		touchpoints: [
			...
			linkToPage("Great Person Expended Reaction", "/pattern-library", "Closest companion when the reward source is a spent Great Person instead of a city event."),
	...
	},
	{
		...
		status: "High-Use Recipe",
		copy: "Use this when a building, improvement, or other content row should directly grant Tourism through the normal yield tables. Tourism does not need a dummy building just because it is a late-game yield; in many cases the regular building or improvement yield tables are the right place to start.",
		deliverables: [
		...
		example: {
			...
			files: [
				...
				snippetFile(
					...
					"Optional text reminder that the player-facing help should match the Tourism rows you just added.",
				),
			],
		},
		touchpoints: [
			linkToSchema("Building_YieldChanges", "Most direct building Tourism patches live here.", "rows"),
			linkToSchema("Improvement_Yields", "Most direct improvement Tourism patches live here.", "rows"),
	...
	},
	{
		...
		touchpoints: [
			linkToLua("GameEvents.CityTrained", "game-event-citytrained-33", "gameEvents", "Training hook for new-unit promotion setup."),
			{ label: "GameEvents.UnitCreated", href: "/lua-api-explorer", note: "Creation hook when scripted units should receive the same starting promotion logic." },
			...
			},
			{
				label: "Unit Created Post-Spawn Setup",
				href: "/pattern-library",
				note: "Read the spawn companion when scripted units need the same normalization pass after they appear.",
			},
			{
	...
	},
	{
		...
		touchpoints: [
			...
			linkToSchema("Policies", "Policy GP modifiers often feed directly into the combined rate calculation.", "rows"),
	...
	},
	{
		...
		deliverables: [
			...
			"A UI dirty/refresh pattern so the city screen re-renders when your data changes.",
		...
		example: {
			...
			files: [
				...
				snippetFile(
					...
					"Minimal UI include target that proves the VFS import path is working.",
				),
			],
		},
		touchpoints: [
...
WHERE Type = 'UNIT_CROSSBOWMAN';`,
	...
	<Language_en_US>
		...
		touchpoints: [
			linkToSchema("Units", "Gameplay row that references the visible text, atlas, and art define keys.", "rows"),
			...
			linkToPage("Civ Icon Maker", "/civ-icon-maker", "Use it when the portrait source art or civ-color treatment still needs a cleaner icon render before atlas registration."),
...
local function syncHarborMarker(pCity)
	...
	pCity:SetNumRealBuilding(iDummyBuilding, shouldHave and 1 or 0)
		...
		touchpoints: [
			...
			linkToSchema("Buildings", "Use a hidden building when the effect should live on a city and integrate with building tables.", "rows"),
...
GameEvents.CityTrained.Add(function(iPlayer, iCity, iUnit)
	...
	pUnit:SetHasPromotion(iPromotion, true)
					...
					"Single training helper that can answer ‘why does this unit get the promotion?’ without duplicating three different checks everywhere.",
				),
			],
		},
		touchpoints: [
			...
			linkToSchema("Buildings", "Building source when the promotion comes from a city structure.", "rows"),
			linkToSchema("Policies", "Policy source when the bonus turns on empirewide after adoption.", "rows"),
			linkToSchema("Leader_Traits", "Trait source when the bonus belongs to a civilization package rather than a building or policy.", "rows"),
			{
				label: "Promotion Grant / Strip Logic",
				href: "/pattern-library",
	...
	},
	{
		...
		status: "High-Use Recipe",
		copy: "Use the city completion hooks that fire after a building or project actually lands. These are better teaching examples than generic lifecycle summaries because they show the city ID, the completed payload, and the easiest follow-up work in one place.",
		deliverables: [
...
GameEvents.CityBoughtPlot.Add(function(iPlayer, iCity, iPlotX, iPlotY, bGold, bFaithOrCulture)
	...
	pPlayer:AddNotification(NotificationTypes.NOTIFICATION_GENERIC, pCity:GetName() .. " converted to a new majority religion.", "City Converts Religion", iPlotX, iPlotY)
		...
		touchpoints: [
			...
			linkToLua("Player:GetCityByID", "player-getcitybyid-146", "methods", "Useful for the founding variant where the hook payload gives a city ID instead of direct city coordinates."),
			linkToLua("City:GetReligiousMajority", "city-getreligiousmajority-233", "methods", "Validate the city’s current majority religion before firing the city payload."),
			...
			linkToPage(
				...
				"Strong companion when the hook needs belief or majority-religion eligibility checks before the city reward fires.",
			),
		],
	},
	{
...
export const wizardCards = [
		...
		example: {
			...
			files: [
				snippetFile(
					...
					"Core SQL file from the scaffold bundle.",
				),
				snippetFile(
	...
	},
	{
		...
		touchpoints: [
			linkToSchema("Civilization_CityNames", "City name table the generator should populate.", "rows"),
			linkToSchema("Civilization_SpyNames", "Spy name table the generator should populate.", "rows"),
			{
				label: "Civilization Starter",
			...
			preview: {
				...
				formDefaults: {
					...
					flavorValues: {
						...
						FLAVOR_NAVAL_TILE_IMPROVEMENT: 3,
						FLAVOR_AIR: 3,
						FLAVOR_EXPANSION: 4,
						FLAVOR_GROWTH: 7,
						FLAVOR_TILE_IMPROVEMENT: 8,
...
export const authoringDatasets = [
	...
	},
	{
		...
		copy: "Local API index spanning 12 method families available for example snippets, parameter hints, and method generator output.",
	},
];
