...
<script>
	...
	let namingSpyList = $state("Tenzin\nPema\nDorje\nLhamo");
	...
	function buildScaffoldIdentity() {
		...
		return {
			...
			primaryColor: `COLOR_PLAYER_${civSuffix}_PRIMARY`,
			secondaryColor: `COLOR_PLAYER_${civSuffix}_SECONDARY`,
	...
	function buildCivilizationScaffoldArchiveBaseName() {
		...
		return `${identity.fileStem}_Scaffold`;
	}
	...
	function buildInteractiveNamingPackExample() {
		const scaffoldStemLabel = "<ScaffoldStem>";
		const textLines = ["<GameData>", "\t<Language_en_US>"];
			...
			files: [
				{
					...
					note: "Replace these naming inserts in the scaffold generator's, or your own, GameDefines.sql file.",
				},
				{
					...
					language: "xml",
					code: textLines.join("\n"),
					note: "Replace these Language_en_US rows in the scaffold generator's, or your own, GameText.xml file.",
				},
			],
		};
	}
	...
	function buildCivilizationScaffoldGameDefinesSql(identity, primaryUnit, secondaryUnique) {
		...
		return [
			"-- Civilization scaffold generated by CMC",
	...
	function buildCivilizationScaffoldArtSql(identity) {
		return [
			...
			"-- Add the DDS files later; the scaffold only registers the expected names.",
			"",
	...
	function buildInteractiveArtBundleExample() {
		...
		return {
			...
			summary: `Scaffold-aligned ArtDefines output for ${identity.civilizationName}, with shared colors${artIncludeUnitArt ? `, ${unit.displayName} unit art` : ""}${artIncludeLandmarkArt ? `, and ${landmark.displayName} ${landmark.kind} art` : ""}.`,
			files: [
				{
					label: "<ScaffoldStem>_ArtDefines.sql",
					path: "Core/<ScaffoldStem>_ArtDefines.sql",
					language: "sql",
					code: buildArtBundleSql(identity, unit, landmark),
					note: "Append this art, audio, and atlas registration block to the scaffold generator's ArtDefines.sql file.",
				},
			],
		};
	}
	...
	function buildInteractiveCivilizationScaffoldExample() {
		...
		return {
			...example,
			title: `${identity.civilizationName} scaffold bundle`,
			...
			files: [
				...
				},
				{
					...
					language: "sql",
					code: modSupportCode,
					note: "Reserved compatibility file for YnAEMP, JFD, or other ecosystem-specific support later.",
				},
				{
	...
	function buildInteractiveLeaderExample() {
		...
		return {
			...
			files: [
				{
					label: "<ScaffoldStem>_GameDefines.sql",
					path: "Core/<ScaffoldStem>_GameDefines.sql",
					language: "sql",
					code: buildLeaderSqlCode(),
					note: " Replace this leader-tuning block to the scaffold generator's, or your own, GameDefines.sql file.",
				},
			],
		};
	}
	...
	function buildStepValidationMap() {
		if (isInteractiveCivScaffold) {
			return {
				...
				"Output bundle": ["ZIP includes the scaffold files only."],
			};
		}
...
<div class="wizard-example-preview">
	...
	{#if isInteractiveCivScaffold && scaffoldDownloadUrl}
		<section class="wizard-download-preview" aria-label="Scaffold download">
			<div class="wizard-download-card">
				<div class="wizard-download-copy">
					<span class="wizard-analysis-eyebrow">Download Scaffold Bundle</span>
					<p>Exports the current civilization scaffold bundle with the generated core files and folder structure.</p>
					<div class="wizard-download-meta">
						<span class="wizard-download-filename">{scaffoldDownloadName}</span>
					</div>
				</div>
				<a class="wizard-download-link" href={scaffoldDownloadUrl} download={scaffoldDownloadName}>
					<strong>Download ZIP</strong>
					<span class="wizard-download-link-meta">{scaffoldDownloadName}</span>
				</a>
			</div>
		</section>
	{/if}
