...
const MAP_INPUT = resolveArg(
  args.map,
  path.join(ROOT, 'static/community/CBRXMapOct62025_V2.civ5map')
);
const SAVE_INPUT = args.save ? resolveArg(args.save, '') : null;
const mapStem = args.name || path.basename(MAP_INPUT).replace(/\.civ5map$/i, '');
const outDir = resolveArg(args.outDir, path.join(ROOT, 'static/community'));

const BASE_OUTPUT = path.join(outDir, `${mapStem}.base.json`);
const STATE_OUTPUT = path.join(outDir, `${mapStem}.state.json`);
  ...
  for (let row = 0; row < height; row += 1) {
    for (let col = 0; col < width; col += 1) {
      ...
      tiles[index] = {
        col,
        row,
        terrainType: buffer.readUInt8(cursor),
        resourceType: buffer.readUInt8(cursor + 1),
        featureTerrainType: buffer.readUInt8(cursor + 2),
        riverData: buffer.readUInt8(cursor + 3),
        elevation: buffer.readUInt8(cursor + 4),
        continent: buffer.readUInt8(cursor + 5),
        featureWonderType: buffer.readUInt8(cursor + 6),
        resourceAmount: buffer.readUInt8(cursor + 7),
      };
      cursor += 8;
    }
  }
...
function findCompressedBlock(buffer) {
  const marker = Buffer.from([0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x78, 0x9c]);
...
async function main() {
  ...
  if (!SAVE_INPUT) {
    console.log('No --save argument provided, skipping state cache generation.');
    return;
  }
