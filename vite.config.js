		...
		configureServer(server) {
			server.middlewares.use("/__local-api/modded-civs-pedia/save", async (req, res) => {
				...
				try {
					...
					const wikiMarkup = String(payload?.wikiMarkup || "");

					if (!slug || !entry || !wikiMarkup) {
						return jsonResponse(res, 400, { ok: false, message: "Missing slug, entry, or wiki markup." });
					}
					...
					await writeFile(wikiFilePath, `${wikiMarkup.trim()}\n`, "utf8");
